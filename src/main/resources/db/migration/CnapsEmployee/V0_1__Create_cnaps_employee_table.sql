...
CREATE OR REPLACE FUNCTION generate_cnaps_employ_custom_ref()
    RETURNS TRIGGER AS
$BODY$
BEGIN
    NEW.registration_number := 'REF-' || LPAD(NEXTVAL('cnaps_employ_ref_sequence')::TEXT, 6, '0');
    RETURN NEW;
END;
$BODY$
    LANGUAGE plpgsql;
...
CREATE TRIGGER insert_reference_number_trigger
    ...
    ON "cnaps_employee"
    FOR EACH ROW
EXECUTE FUNCTION generate_cnaps_employ_custom_ref();